                    ...
                    fs.readFile("usernames.json", "utf-8", (err, data) => {
                        ...
                        setTimeout(() => {
                            fs.readFile("usernames.json", "utf-8", (err, data) => {
                                ...
                                var key = getKeyByValue(usernames, [`${message}`])

                                
                                const BASE_URL3 = `https://api.twitch.tv/helix/users?login=${key}`
                                let config = {
                                    headers: {
                                ...
                                axios.default.get(BASE_URL3, config).then((resp) => {
                                
                                    var userId = resp.data.data[0].id
                                    
                                    const BASE_URL1 = `https://api.twitch.tv/helix/users/follows?from_id=${userId}&to_id=${configTwitch.toId}`
                                    let config = {
                                        headers: {
                                            'Authorization': `Bearer ${configTwitch.authToken}`,
                                            "Client-ID": configTwitch.clientId
...
orion.on('chat', function (channel, user, message, _self) {
    if (_self) return;
    run(orion, channel, user, message, _self, publicWS);
});
    
function run(orion, channel, user, message, _self, ws) {
                            ...
                            if(usernames[user['display-name'].toLocaleLowerCase()] != undefined) {
                                ...
                                if (user['display-name'].toLocaleLowerCase() in usernames && values.includes(args[0]) && values.length == 2) {
                                    ...
                                    if (indexOfMCName == 1) {
                                        ...
                                        ws.send("UNWHITELIST " + usernames[user['display-name'].toLocaleLowerCase()][1])
                                    }
                                    if (indexOfMCName == 0) {
                                        newNames = usernames[user['display-name'].toLocaleLowerCase()][1]
                                        ws.send("UNWHITELIST " + usernames[user['display-name'].toLocaleLowerCase()][0])
                                    }
...
function getKeyByValue(object, value) {
    for (var prop in object) {
        if (object.hasOwnProperty(prop)) {
            if (_.isEqual(object[prop], value))
            return prop;
        }
    }
}